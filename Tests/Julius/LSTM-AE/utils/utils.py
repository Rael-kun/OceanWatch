...
def plotMapBackground(ax, edges, c=0):
    ...
    if c == 0:
        img = mpimg.imread("plots/areaBornholm.png")
    elif c == 1:
        img = mpimg.imread("plots/areaSjælland.png")
    else:
        img = mpimg.imread("plots/Anholt.png")
...
def plotTrack(data, speed, ax, color=None, lsty="solid", insertSpeed=False, label=None):
    ...
    for ii in range(0, seq_len - 1):
        segii = segments[ii]
        (lii,) = ax.plot(
            segii[:, 0], segii[:, 1], color=colors[ii], linestyle=lsty, label=label
        )
...
def convertCourseToTrig(course):
    ...
    trig = np.concatenate(
        [
            np.expand_dims(np.sin(np.deg2rad(course)), axis=1),
            np.expand_dims(np.cos(np.deg2rad(course)), axis=1),
        ],
        axis=1,
    )
