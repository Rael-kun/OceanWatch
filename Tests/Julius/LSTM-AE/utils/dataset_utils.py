...
class AISDataset(torch.utils.data.Dataset):
    ...
    def __len__(self):
        return min(self.datasetN, 3000)

    def __getitem__(self, idx):

        index = self.indicies[idx % len(self)]
        ...
        label = (
            np.where(typeName == self.classnames)[0][0]
        ...
        targets = torch.tensor(
            tmpdf[["lat", "lon", "speed", "course"]].values, dtype=torch.float
        )
        inputs = (targets - self.mean) / self.std

        return inputs[:, :NUM_FEATURES_TO_USE]
    ...
    def reconstruct(self, encodedTrack):
        recon = (
            encodedTrack * self.std[:NUM_FEATURES_TO_USE]
            + self.mean[:NUM_FEATURES_TO_USE]
        )
