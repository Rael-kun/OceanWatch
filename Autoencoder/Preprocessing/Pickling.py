...
def PickleData(dataPath, trainPath, valPath, testPath, outTrainPath, outValPath, outTestPath):
    ## PARAMS
    LAT, LON, SOG, HEADING, PATHTIME, TIMESTAMP, MMSI, PATH = list(range(8))
    ...
    for csv_filename in l_csv_filename:
        ...
        for i in range(len(f["mmsi"])):
            ...
            l_l_msg.append([float(row["alat"]), float(row["alon"]), float(row["aspeed"]),
                                float(row["aheading"]), int(row["pathTime"]),
                                int(row["timePassed"]), int(row["mmsi"]),
                            int(row["pathIni"])])
    ...
    if(len(m_msg_train) != 0):
        for v_msg in tqdm(m_msg_train):
            ...
            if (pathIni not in list(Vs_train.keys())):
                Vs_train[pathIni] = {'mmsi': mmsi, 'traj': np.empty((0,7))}
            Vs_train[pathIni] = {'mmsi': mmsi, 'traj': np.concatenate((Vs_train[pathIni]['traj'], np.expand_dims(v_msg[:7],0)), axis = 0)}
        for key in tqdm(list(Vs_train.keys())):
            mmsi = int(Vs_train[key]['traj'][0][6])
    ...
    if(len(m_msg_valid) != 0):
        for v_msg in tqdm(m_msg_valid):
            mmsi = int(v_msg[MMSI])
            pathIni = int(v_msg[PATH])
            if (pathIni not in list(Vs_valid.keys())):
                Vs_valid[pathIni] = {'mmsi': mmsi, 'traj': np.empty((0,7))}
            Vs_valid[pathIni] = {'mmsi': mmsi, 'traj': np.concatenate((Vs_valid[pathIni]['traj'], np.expand_dims(v_msg[:7],0)), axis = 0)}
        for key in tqdm(list(Vs_valid.keys())):
            mmsi = int(Vs_valid[key]['traj'][0][6])
    ...
    if(len(m_msg_test) != 0):
        for v_msg in tqdm(m_msg_test):
            mmsi = int(v_msg[MMSI])
            pathIni = int(v_msg[PATH])
            if (pathIni not in list(Vs_test.keys())):
                Vs_test[pathIni] = {'mmsi': mmsi, 'traj': np.empty((0,7))}
            Vs_test[pathIni] = {'mmsi': mmsi, 'traj': np.concatenate((Vs_test[pathIni]['traj'], np.expand_dims(v_msg[:7],0)), axis = 0)}
        for key in tqdm(list(Vs_test.keys())):
            mmsi = int(Vs_test[key]['traj'][0][6])
    ...
    for filename, filedict in zip([pkl_filename_train,pkl_filename_valid,pkl_filename_test],
        ...
        if(filename != ""):
            ...
            with open(filename,"wb") as f:
                ...
                print("Total number of tracks: ", len(filedict), " for ", filename)
